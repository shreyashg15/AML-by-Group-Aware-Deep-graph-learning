...
# - Link Reconstruction metrics (Accuracy, Precision, Recall, F1, AUC-ROC) – internal only
# - Node-level Fraud Detection classifier on top of embeddings (if labels available)
# - Fraud Detection metrics stored in meta file and SHOWN in UI (even on reload)
...
def visualize_tree_graph_static(G, fraud_nodes):
    """Static (non-interactive) fraud tree graph."""
...
def visualize_graph(G, fraud_nodes, pos, groups, mode="Static", transactions=None):
    """(Currently unused) Interactive graph helper."""
    edge_x, edge_y = [], []
    for edge in G.edges():
        x0, y0 = pos[edge[0]]
        x1, y1 = pos[edge[1]]
...
def visualize_tree_graph_plotly(G, fraud_nodes):
    """(Not used now) Interactive tree graph."""
...
def load_or_train_model(
    ...
    with torch.no_grad():
        ...
        metrics = {
            "Accuracy": acc,  # Link reconstruction accuracy (internal)
            "Precision": prec,
            "Recall": rec,
            "F1 Score": f1,
            "AUC-ROC": auc,
    ...
    if label_col is not None:
        ...
        if node_labels.sum() > 0 and node_labels.sum() < len(node_labels):
            try:
                ...
                # Show fraud metrics at training time
...
# -------------------------
    ...
    acc_val = metrics.get("Fraud_Accuracy")
    ...
    # ---- Show ONLY Fraud Detection metrics (and do it also on reload) ----
    st.subheader("Fraud Detection Metrics (GNN + Classifier)")
    if metrics.get("Fraud_Accuracy") is not None:
        fraud_metrics_table = pd.DataFrame(
            {
                "Accuracy": [metrics["Fraud_Accuracy"]],
                "Precision": [metrics["Fraud_Precision"]],
                "Recall": [metrics["Fraud_Recall"]],
                "F1 Score": [metrics["Fraud_F1"]],
                "AUC-ROC": [metrics["Fraud_AUC"]],
            }
        ).T.rename(columns={0: "Score"})
        st.table(fraud_metrics_table)
    else:
        st.info("Fraud Detection metrics are not available for this dataset (no usable labels).")
