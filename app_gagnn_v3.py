...
def load_or_train_model(df: pd.DataFrame, force_retrain=False, epochs=100, lr=0.001,
    ...
    # --- NEW: Save dataset fingerprint to extra folder ---
    Path("data_fingerprints").mkdir(parents=True, exist_ok=True)
    with open(Path("data_fingerprints") / f"{fingerprint}.txt", "w") as f:
        f.write(fingerprint)
    model_path = Path("models") / f"{fingerprint}_gagnn.pth"
    pred_path = Path("models") / f"{fingerprint}_node_predictions.csv"
    meta_path = Path("models") / f"{fingerprint}_meta.json"

    custom_model_path = Path("models") / f"{custom_name}.pth" if custom_name else None
    custom_pred_path = Path("models") / f"{custom_name}_node_predictions.csv" if custom_name else None
    ...
    money_in_dict = df.groupby('target')["amount"].sum().to_dict()
    money_out_dict = df.groupby('source')["amount"].sum().to_dict()
    preds_df["money_in"] = preds_df["node"].map(money_in_dict).fillna(0)
    preds_df["money_out"] = preds_df["node"].map(money_out_dict).fillna(0)
    preds_df["total_amount"] = preds_df["money_in"] + preds_df["money_out"]

    # Reorder columns so money_in/money_out appear before total_amount
    cols = preds_df.columns.tolist()
    total_idx = cols.index("total_amount")
    for col in ["money_out", "money_in"]:
        cols.insert(total_idx, cols.pop(cols.index(col)))
    preds_df = preds_df[cols]
    # Reorder so money_in/money_out appear before total_amount (once you add total_amount elsewhere)
    if "total_amount" in preds_df.columns:
        cols = preds_df.columns.tolist()
        total_idx = cols.index("total_amount")
        for col in ["money_out", "money_in"]:
            cols.insert(total_idx, cols.pop(cols.index(col)))
...
def plot_graph_with_predictions(df: pd.DataFrame, preds_df: pd.DataFrame, src_col="source", tgt_col="target", amount_col="amount", mode="Interactive", threshold=0.5):
    ...
    # Scale positions to spread nodes apart
    spread_factor = 3# Increase for more spacing
    pos = {node: (coords[0] * spread_factor, coords[1] * spread_factor) for node, coords in pos.items()}
    edge_x, edge_y = [], []
    for e in G.edges():
        x0, y0 = pos[e[0]]
        x1, y1 = pos[e[1]]
        edge_x.extend([x0, x1, None])
        edge_y.extend([y0, y1, None])
    edge_trace = go.Scatter(x=edge_x, y=edge_y, line=dict(width=0.5, color='rgba(50,50,50,0.4)'), hoverinfo='none', mode='lines')
    # --- NEW: Add arrow annotations for direction ---
    arrows = []
    for e in G.edges():
        arrows.append(go.layout.Annotation(
            ax=pos[e[0]][0], ay=pos[e[0]][1],
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
        yaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
        annotations=list(fig.layout.annotations) + arrows   # attach all arrows here
    )
    # --- NEW: attach arrow annotations ---
    fig.update_layout(annotations=list(fig.layout.annotations) + arrows)
...
def main():
    ...
    **Notes**
    ...
    - Trained model and predictions are saved under `models/` with a dataset fingerprint.
    ...
    fp_fingerprint = Path("models") / f"{dataset_fingerprint(df)}_node_predictions.csv"
    ...
    fp_fixed = Path("models") / "node_predictions.csv"
    if fp_fixed.exists():
    ...
    mp_fixed = Path("models") / "gagnn_model.pth"
    if mp_fixed.exists():
    ...
    # custom files
    if custom_name:
        mp_custom = Path("models") / f"{custom_name}.pth"
        pp_custom = Path("models") / f"{custom_name}_node_predictions.csv"
        if pp_custom.exists():
